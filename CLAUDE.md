...
- **Relays can target any folder in the shared drive.** The sync keeps every shortcut row whose Drive shortcut still exists; the sync-folder/`[relay-ignore]` filter only limits which shortcuts it *adds* on its own.
